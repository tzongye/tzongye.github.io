...
class CalendarService {
    constructor() {
        this.dongGongCalculator = new DongGongCalculator_1.DongGongCalculator();
     ...
     * 生成單日的日曆資訊
     */
    generateDayInfo(date) {
        // 獲取農曆資訊
        const lunarInfo = this.lunarService.getLunarInfo(date);
        // 執行董公擇日分析
        const analysis = this.dongGongCalculator.performSevenStepAnalysis(date, '嫁娶');
        // 判斷黃道/黑道
        ...
        // 生成快速摘要
        const quickSummary = this.generateQuickSummary(analysis.step3_buildAnalysis.name, analysis.step7_comprehensiveJudgment.summary);
        // 確定色彩代碼
        const colorCode = this.determineColorCode(analysis.step7_comprehensiveJudgment.level, isHuangDao, isHeiDao, analysis.step6_specialDays);
        return {
            gregorianDate: date,
            lunarDate: lunarInfo.lunarDate,
            twelveBuild: analysis.step3_buildAnalysis,
            godsEvils: analysis.step4_godsEvilsCheck,
            specialDay: analysis.step6_specialDays,
            isHuangDao,
            isHeiDao,
     ...
     */
    isHeiDaoDay(build) {
        const heiDaoBuilds = ['滿', '平', '破', '收'];
        return heiDaoBuilds.includes(build);
    }
    /**
     * 生成快速摘要
     */
    generateQuickSummary(build, summary) {
        return `${build}日 ${summary}`;
    }
    /**
     * 確定色彩代碼
     */
    determineColorCode(level, isHuangDao, isHeiDao, specialDay) {
        // 特殊日期優先
