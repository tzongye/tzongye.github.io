...
class LunarService {
    ...
    gregorianToLunar(date) {
        try {
            const solar = lunar_javascript_1.Solar.fromDate(date);
     ...
     * 計算五行強弱（簡化版本）
     */
    calculateElementStrength(date, element) {
     ...
     * 分析五行關係
     */
    analyzeWuXingRelation(element, date) {
        const relations = {
    ...
    isValidDate(date) {
        try {
            lunar_javascript_1.Solar.fromDate(date);
            return true;
        }
        catch (error) {
            return false;
     ...
     * 獲取農曆月份的天數
     */
    getLunarMonthDays(year, month, isLeapMonth = false) {
        try {
            // 簡化實現，返回合理的天數範圍
...
exports.LunarService = LunarService;
//# sourceMappingURL=LunarService.js.map