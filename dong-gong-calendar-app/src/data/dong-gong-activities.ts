...
// 董公擇日完整事項清單（基於傳統分類）
export const DONG_GONG_ACTIVITIES: DongGongActivity[] = [
  // 人事類
  {
    id: 'marry',
    name: '嫁娶',
...
// 董公擇日傳統分類
export const DONG_GONG_CATEGORIES = {
  人事: {
    name: '人事類',
    description: '人生重要事項',
...
// 現代生活對應（幫助用戶理解）
export const MODERN_ACTIVITY_MAPPING = {
  '嫁娶': ['結婚', '婚禮', '喜宴'],
  '納采': ['訂婚', '下聘', '提親'],
  '動土': ['建築', '裝修', '挖掘', '施工'],
  '入宅': ['搬家', '喬遷', '入厝'],
...
// 根據活動ID獲取活動資訊
export function getActivityById(id: string): DongGongActivity | undefined {
  return DONG_GONG_ACTIVITIES.find(activity => activity.id === id);
}

// 根據分類獲取活動列表
export function getActivitiesByCategory(category: string): DongGongActivity[] {
  return DONG_GONG_ACTIVITIES.filter(activity => activity.category === category);
}

// 獲取所有分類
export function getAllCategories(): string[] {
  return Object.keys(DONG_GONG_CATEGORIES);
}

// 根據建星獲取適合的活動
export function getActivitiesByBuild(build: TwelveBuild): DongGongActivity[] {
  return DONG_GONG_ACTIVITIES.filter(activity => 
    activity.buildPreference.includes(build)
  );
}

// 根據建星獲取不適合的活動
export function getAvoidActivitiesByBuild(build: TwelveBuild): DongGongActivity[] {
  return DONG_GONG_ACTIVITIES.filter(activity => 
    activity.avoidBuilds.includes(build)
  );
}