...
const SPECIAL_INAUSPICIOUS_DAYS = [
  // 建星配地支的特殊大凶組合
  { build: '建', branch: '巳', name: '建巳日', severity: 'extreme' as const, description: '建巳日大凶，諸事不宜' },
  { build: '平', branch: '巳', name: '平巳日', severity: 'extreme' as const, description: '平巳日大凶，百事皆忌' },
  { build: '破', branch: '申', name: '破申日', severity: 'extreme' as const, description: '破申日大凶，破敗之象' },
  { build: '危', branch: '酉', name: '危酉日', severity: 'extreme' as const, description: '危酉日大凶，危險重重' },
  { build: '收', branch: '戌', name: '收戌日', severity: 'severe' as const, description: '收戌日凶，收煞入墓' },
  { build: '開', branch: '辰', name: '開辰日', severity: 'medium' as const, description: '開辰日需謹慎，雖開而有阻' },
  
  // 其他特殊凶日
  { build: '滿', branch: '亥', name: '滿亥日', severity: 'severe' as const, description: '滿亥日凶，滿而必溢' },
  { build: '定', branch: '巳', name: '定巳日', severity: 'medium' as const, description: '定巳日需謹慎，定中有變' },
  { build: '執', branch: '亥', name: '執亥日', severity: 'medium' as const, description: '執亥日需謹慎，執而不通' },
  { build: '閉', branch: '丑', name: '閉丑日', severity: 'severe' as const, description: '閉丑日大凶，閉塞不通' }
];

// 特殊大吉日配置
const SPECIAL_AUSPICIOUS_DAYS = [
  // 建星配地支的特殊大吉組合
  { build: '定', branch: '戌', name: '定戌日', level: 'excellent' as const, description: '定戌日大吉，定而有成，百事皆宜' },
  { build: '定', branch: '午', name: '定午日', level: 'excellent' as const, description: '定午日大吉，定中得火，光明正大' },
  { build: '成', branch: '子', name: '成子日', level: 'excellent' as const, description: '成子日大吉，成功之始' },
  { build: '成', branch: '午', name: '成午日', level: 'excellent' as const, description: '成午日大吉，成功光明' },
  { build: '開', branch: '子', name: '開子日', level: 'excellent' as const, description: '開子日大吉，開創新局' },
  { build: '開', branch: '午', name: '開午日', level: 'excellent' as const, description: '開午日大吉，開明盛大' },
  
  // 其他特殊吉日
  { build: '除', branch: '寅', name: '除寅日', level: 'good' as const, description: '除寅日吉，除舊迎新' },
  { build: '除', branch: '申', name: '除申日', level: 'good' as const, description: '除申日吉，除煞得金' },
  { build: '滿', branch: '辰', name: '滿辰日', level: 'good' as const, description: '滿辰日吉，滿而有庫' },
  { build: '執', branch: '寅', name: '執寅日', level: 'good' as const, description: '執寅日吉，執而有生' }
];
 ...
 * 檢查四絕四離日
 */
export function checkSiJueSiLi(date: Date): SiJueSiLiResult {
  try {
 ...
 * 檢查特殊大凶日
 */
export function checkSpecialInauspiciousDay(build: TwelveBuild, dayBranch: string): SpecialDateResult {
  const specialDay = SPECIAL_INAUSPICIOUS_DAYS.find(
    day => day.build === build && day.branch === dayBranch
  );
 ...
 * 檢查特殊大吉日
 */
export function checkSpecialAuspiciousDay(build: TwelveBuild, dayBranch: string): SpecialDateResult {
  const specialDay = SPECIAL_AUSPICIOUS_DAYS.find(
    day => day.build === build && day.branch === dayBranch
  );
 ...
 * 檢查煞入中宮
 */
export function checkShaRuZhongGong(date: Date): ShaRuZhongGongResult {
  try {
    const solar = require('lunar-javascript').Solar.fromDate(date);
    const lunar = solar.getLunar();
    ...
    const yearRules = SHA_RU_ZHONG_GONG_RULES.yearlyRules[yearGan as keyof typeof SHA_RU_ZHONG_GONG_RULES.yearlyRules];
 ...
 * 檢查三煞方位
 */
export function checkSanSha(date: Date, direction?: string): SanShaResult {
  try {
 ...
 * 綜合特殊日期檢查
 */
export function checkAllSpecialDates(date: Date, build: TwelveBuild): {
  ...
  try {
    ...
    let type: 'extremely_auspicious' | 'auspicious' | 'neutral' | 'inauspicious' | 'extremely_inauspicious' = 'neutral';
    let summary = '';
    const recommendations: string[] = [];
    const warnings: string[] = [];
