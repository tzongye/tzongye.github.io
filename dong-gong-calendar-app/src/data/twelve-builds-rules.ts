...
export interface TwelveBuildRule {
  ...
  // 與其他建星的關係
  relationships: {
    opposite?: TwelveBuild;  // 對沖建星
    harmonious?: TwelveBuild[];  // 和諧建星
    conflicting?: TwelveBuild[];  // 衝突建星
  };
...
// 十二建星完整規則資料庫
export const TWELVE_BUILDS_RULES: Record<TwelveBuild, TwelveBuildRule> = {
  // 黑道日 - 建滿平收黑
  '建': {
    name: '建',
    category: 'auspicious',
...
// 建星分類查詢函數
export function getBuildsByCategory(category: 'yellow_path' | 'black_path' | 'usable' | 'inauspicious'): TwelveBuild[] {
  return Object.values(TWELVE_BUILDS_RULES)
    .filter(rule => rule.type === category)
    .map(rule => rule.name);
}

// 獲取黃道吉日
export function getYellowPathBuilds(): TwelveBuild[] {
  return getBuildsByCategory('yellow_path');
}

// 獲取黑道日
export function getBlackPathBuilds(): TwelveBuild[] {
  return getBuildsByCategory('black_path');
}

// 獲取可用日
export function getUsableBuilds(): TwelveBuild[] {
  return getBuildsByCategory('usable');
}

// 獲取不吉祥日
export function getInauspiciousBuilds(): TwelveBuild[] {
  return getBuildsByCategory('inauspicious');
}

// 根據建星獲取詳細規則
export function getBuildRule(build: TwelveBuild): TwelveBuildRule {
  return TWELVE_BUILDS_RULES[build];
}

// 根據事項獲取適合的建星
export function getSuitableBuildsForActivity(activity: string): TwelveBuild[] {
  return Object.values(TWELVE_BUILDS_RULES)
...
// 根據事項獲取不適合的建星
export function getUnsuitableBuildsForActivity(activity: string): TwelveBuild[] {
  return Object.values(TWELVE_BUILDS_RULES)
...
// 驗證建星分類是否正確
export function validateBuildFormula(): boolean {
  ...
  return (
    JSON.stringify(yellowPath.sort()) === JSON.stringify(BUILD_FORMULA.yellow_path.sort()) &&
    JSON.stringify(blackPath.sort()) === JSON.stringify(BUILD_FORMULA.black_path.sort()) &&
    JSON.stringify(usable.sort()) === JSON.stringify(BUILD_FORMULA.usable.sort()) &&
    JSON.stringify(inauspicious.sort()) === JSON.stringify(BUILD_FORMULA.inauspicious.sort())
  );
}// 傳統口訣

export const TRADITIONAL_VERSES = {
...
// 獲取建星的詳細分析
export function getBuildAnalysis(build: TwelveBuild): {
