...
exports.calculateBaZiInfo = calculateBaZiInfo;
exports.analyzeBodyStrength = analyzeBodyStrength;
exports.analyzeYongShen = analyzeYongShen;
exports.analyzePattern = analyzePattern;
exports.analyzeDateCompatibility = analyzeDateCompatibility;
exports.getWuXing = getWuXing;
exports.checkWuXingRelation = checkWuXingRelation;
// 天干五行屬性
const HEAVENLY_STEM_WUXING = {
    '甲': '木', '乙': '木',
    '丙': '火', '丁': '火',
    '戊': '土', '己': '土',
    '庚': '金', '辛': '金',
 ...
 * 計算八字基本資訊
 */
function calculateBaZiInfo(birthInfo) {
    try {
 ...
 * 身強身弱判斷
 */
function analyzeBodyStrength(baZiInfo) {
    let score = 50; // 基礎分數
    const factors = [];
    // 1. 月令支持度（最重要，佔40%）
    ...
    // 3. 生助五行支持度（佔20%）
    const shengWuXing = Object.keys(WUXING_RELATIONS.sheng).find(key => WUXING_RELATIONS.sheng[key] === dayMasterWuXing);
    if (shengWuXing) {
 ...
 * 用神忌神分析
 */
function analyzeYongShen(baZiInfo, bodyStrength) {
    const dayMasterWuXing = HEAVENLY_STEM_WUXING[baZiInfo.dayMaster];
    let yongShen = [];
    let jiShen = [];
    let analysis;
    ...
    if (bodyStrength.strength === 'strong') {
        ...
        const guanWuXing = Object.keys(WUXING_RELATIONS.ke).find(key => WUXING_RELATIONS.ke[key] === dayMasterWuXing); // 剋日主（官殺）
        yongShen = [shiWuXing, guanWuXing].filter(Boolean);
        jiShen = [dayMasterWuXing]; // 忌比劫
        // 加入印星為忌神
        const yinWuXing = Object.keys(WUXING_RELATIONS.sheng).find(key => WUXING_RELATIONS.sheng[key] === dayMasterWuXing);
        if (yinWuXing)
            jiShen.push(yinWuXing);
        ...
        const yinWuXing = Object.keys(WUXING_RELATIONS.sheng).find(key => WUXING_RELATIONS.sheng[key] === dayMasterWuXing);
        if (yinWuXing)
            yongShen.push(yinWuXing); // 印星
        // 忌神為官殺、食傷、財星
        const guanWuXing = Object.keys(WUXING_RELATIONS.ke).find(key => WUXING_RELATIONS.ke[key] === dayMasterWuXing);
        const shiWuXing = WUXING_RELATIONS.ke[dayMasterWuXing];
        jiShen = [guanWuXing, shiWuXing].filter(Boolean);
 ...
 * 格局分析
 */
function analyzePattern(baZiInfo) {
    var _a;
    ...
    // 簡化的格局判斷
    const shiShen = (_a = SHISHEN_RELATIONS[dayMaster]) === null || _a === void 0 ? void 0 : _a[monthGan];
    let pattern;
    let type;
    let description;
    let characteristics;
 ...
 * 日期配合分析
 */
function analyzeDateCompatibility(baZiInfo, bodyStrength, yongShen, dateGanZhi // 暫時使用 any，因為 GanZhiInfo 可能結構不同
) {
    ...
    // 綜合計算配合度
    compatibility = Math.round(dayMasterSupport * 0.4 +
        yongShenSupport * 0.4 +
        jiShenAvoidance * 0.2);
    // 確保分數在合理範圍內
 ...
 * 檢查五行關係
 */
function checkWuXingRelation(wuxing1, wuxing2) {
    if (wuxing1 === wuxing2)
        return '同類';
    if (WUXING_RELATIONS.sheng[wuxing1] === wuxing2)
        return '相生';
    if (WUXING_RELATIONS.ke[wuxing1] === wuxing2)
        return '相剋';
    return '無關';
}
